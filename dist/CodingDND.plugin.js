        ...
        const CodingDND = (Plugin, Library) => {
          ...
          return class CodingDND extends Plugin {
            constructor() {
              ...
              const settings_from_config = Bapi.loadData(
                "CodingDND",
                "settings"
              );
            ...
            async load_user() {
              ...
              const get_user = () =>
                Bapi.findModuleByProps("getCurrentUser").getCurrentUser();
              let user;
              // try getting the user every second until successful
              for (let attempt = 0; attempt < 10; attempt++) {
                this.log_func(
                  `Attempt ${attempt + 1}: trying to get the user id`
                );
              ...
              return Object.keys(this.settings[setting_section_name]).map(
                (name) => {
                  return new Settings.Switch(
                    ...
                    (new_value) => on_change(name, new_value)
                  );
                }
              );
            }
            ...
            get status_menu() {
              ...
              return new Settings.SettingGroup("Statuses", {
                ...
                new Settings.Dropdown(
                  ...
                  this.settings.active_status,
                  statuses,
                  (new_status) => (this.settings.active_status = new_status)
                ),
                new Settings.Dropdown(
                  "Inactive status",
            ...
            untrack(name) {
              ...
              this.targets = this.targets.filter(
                (value) => !actual_names.includes(value)
              );
            }
            /**
            ...
            add_custom_task(name) {
              ...
              const target_processes_settings_group =
                this.settings_panel.childNodes[0]; // settings_panel will be defined when this is called
